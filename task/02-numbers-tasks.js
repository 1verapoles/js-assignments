...
function roundToPowerOfTen(num, pow) {
       /* var  mju = String(num);
        for (var i = -1; i >= (pow*(-1)); i--) {
            if (i = (pow*(-1)) && mju[i]>=5) {mju[i-1] = mju[i-1]+1;}
            mju[i] = 0; 
             }
           return parseFloat(mju);  */
...
function toNumber(value, def) {
   /*if (value!==null && value!==undefined && typeof value !== 'string' && typeof parseFloat(value) !== NaN) {return Number(value);}//if (parseFloat(value) !== NaN) {return parseFloat(value);}
    else {return def;}*/
throw new Error('Not implemented');
}
